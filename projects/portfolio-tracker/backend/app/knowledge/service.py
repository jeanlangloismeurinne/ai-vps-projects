...
Substrat commun de tous les agents V2. Trois opérations du LLM Wiki Pattern (§6.1) :
  ...
  - **query_knowledge** : recherche sur la version COURANTE des entrées (superseded_by IS NULL AND
    NOT is_deleted). Recherche TEXTE (ILIKE multi-termes) — le vectoriel (embedding vector(768)) est
    prévu au schéma mais Ollama/nomic-embed n'est pas encore provisionné sur le VPS : dégradation
    documentée, bascule triviale quand les embeddings seront peuplés.
...
async def query_knowledge(
    ...
    *,
    ...
    Avec `query` : recherche TEXTE multi-termes (ILIKE sur content+title+tags), classée par nombre de
    termes matchés puis fiabilité. Substitut au vectoriel tant qu'Ollama/embeddings ne sont pas
    provisionnés (voir docstring module). L'anti-doublon du search-worker (query_knowledge avant store)
    passe par ici.
    """
    if not query or not query.strip():
        rows = await get_current_entries(
    ...
    # score de pertinence = nb de termes présents dans content|title|tags (ILIKE)
    hit_exprs: list[str] = []
    for term in terms:
    ...
    sql = f"""
        ...
        FROM knowledge_entries
        WHERE {_CURRENT} AND {scope} AND reliability_score >= $2{type_clause}
    ...
    return [r for r in rows if r.get("relevance", 0) > 0] or rows
