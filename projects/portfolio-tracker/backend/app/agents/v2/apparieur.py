...
_APPARIEUR_SYSTEM_PROMPT = (
    ...
    "champs recopiés de la table, des opérateurs (`+ - * / ( )`), des nombres entiers de "
    "conversion, et un DÉCALAGE D'EXERCICE optionnel (ci-dessous). Aucun mot de français, aucune "
    "virgule d'énumération, aucun « Pour chaque… », aucune explication : tout ce qui s'explique va "
    "dans `hypotheses`, dont c'est exactement le rôle. Un mot commençant par une majuscule dans la "
    "formule est lu comme un nom de champ et sera cherché dans la table — une phrase y fait donc "
    "refuser toute la carte.\n"

    "RÉFÉRENCER LE MÊME CHAMP À UN EXERCICE ANTÉRIEUR — `Champ[-1]`. Pour une CROISSANCE, une "
    "VARIATION ou une évolution d'un exercice au suivant, tu n'as pas de champ « exercice "
    "précédent » à chercher, et tu ne dois JAMAIS en inventer un (`Revenues_previous_year` n'existe "
    "pas et fait refuser la carte). Écris le MÊME nom de champ avec un décalage entre crochets : "
    "`Champ` (ou `Champ[0]`) est le dernier exercice déposé, `Champ[-1]` le précédent, `Champ[-2]` "
    "celui d'avant. Une progression annuelle du chiffre d'affaires s'écrit donc, si le champ déposé "
    "est `Revenues` : `(Revenues[0] - Revenues[-1]) / Revenues[-1]`. Tu déclares `Revenues` UNE "
    "fois dans `concepts` (le décalage porte sur le nom, il ne crée pas un nom nouveau). Le décalage "
    ...
    "(`(Revenues[0] - Revenues[-1]) / Revenues[-1]` pour la dernière progression), et dis dans une "
    "hypothèse qu'elle vaut pour chacun des exercices déposés (en citant leur nombre) : le fait "
    "produit porte la progression la plus récente, et la profondeur affichée garantit qu'elle se "
    "calcule aussi sur les exercices antérieurs. Si `+A×K` montre au moins deux exercices, "
    "l'ingrédient est SERVI — ce n'est pas un `indisponible`. N'écris JAMAIS une formule par année.\n"
...
def message_reparation(refus: str, absents: list[str], inventaire: Collection[str]) -> str:
    ...
    return (
        ...
        + "\nRAPPEL DE FORME : `formule` ne contient QUE des noms de champs recopiés de la table, "
          "des opérateurs, des entiers, et un décalage d'exercice `Champ[-1]` — jamais un mot de "
