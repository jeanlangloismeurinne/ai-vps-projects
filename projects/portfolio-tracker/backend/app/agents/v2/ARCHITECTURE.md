...
| `note_flash.py` | **Note flash** (#90, branchée au flux #91) : lit un dépôt « à qualifier » (formulaire + communiqués EX-99), le range dans le catalogue d'événements en citant le passage ; pont de citation littérale ; persistance append-only (migration 050) et relecture pour l'horloge par question |
...
| LA NOTE FLASH BRANCHÉE AU FLUX (lot 7, arbitrage 2026-09-26 option c) : un seul geste, `lire_les_depots_en_attente` (détenteur unique), appelé à DEUX moments — chaque matin pour tout titre suivi (job `notes_flash_matin`, 7 h Paris, dépense sous `v2_auto_enabled` : coupé ⟹ recensement gratuit et signal des seuls dépôts publiés depuis la veille) et à chaque passage de la chaîne (`executer_chaine`, `boucler_renvois` avant le dossier) · un refus, une panne ou une lecture qui ne rend pas la main sur UN dépôt est un refus nommé, jamais la fin du passage · titre hors EDGAR dit « hors flux » | `check_note_flash_branchement.py` + `negatif_note_flash_branchement.sh` |
