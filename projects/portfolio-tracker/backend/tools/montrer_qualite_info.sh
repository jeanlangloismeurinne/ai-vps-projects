#!/usr/bin/env bash
# Lanceur de `tools/montrer_qualite_info.py` — montre la qualité d'information d'un dossier telle
# qu'elle se mesure au niveau 1 du parcours. Versionné : l'invocation EST une partie de la mesure.
#
...
exec docker run --rm --network coolify -v "$PWD:/app:ro" \
  ...
  "$IMG" python tools/montrer_qualite_info.py "$@"
