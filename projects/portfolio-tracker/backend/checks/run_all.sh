...
for f in checks/check_*.py; do
  ...
  # mandats s'écrivent vraiment) et `check_framework_persist` (lot 3 maillon 2 — les réponses/
  # dispenses s'écrivent vraiment). Tous dans une transaction ROLLBACK, sans résidu. Ils exigent le
  # réseau `coolify` + la vraie URL de base. Sans elles ils SORTENT EN ÉCHEC au lieu de sauter la
  # section — une mesure incomplète ne doit jamais passer pour un 0 (`feedback_check_degrade_en_sortant_a_zero`).
  ...
  net=none; extra=()
  case "$n" in
    check_entry_nature|check_collecte_persist|check_framework_persist|check_appariement_persist|check_manager_persist|check_datation|check_comite_persist|check_note_flash_persist)
      net=coolify
