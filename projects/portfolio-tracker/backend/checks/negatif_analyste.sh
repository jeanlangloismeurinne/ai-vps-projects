...
"$SRC¦        _, rang, _ = derive_synthesis_reliability(tiers)¦        rang = _plus_faible(tiers)¦UN CRAN SOUS"
"$SRC¦            \"mesure\", \"evenement\", \"interpretation\"} else \"interpretation\"¦            \"mesure\", \"evenement\", \"interpretation\"} else \"mesure\"¦nature forte n'est concédée"
...
"$SRC¦    if question.nature_attendue in natures:¦    if True:¦\`repondu\` est FERMÉ quand aucune entry"
