"""
Schéma Pydantic versionné de la SORTIE / POST-MORTEM / CALIBRATION (§11, §12, migration 027) —
...
Réutilise `Strict`/`NonEmptyRefs` d'analysis_v2 (G1). `valider_postmortem_couvre` est le contrôle
inter-JSON (comme `valider_pont_risques_hypotheses` en §8.5) : un post-mortem ne peut pas oublier
une hypothèse figée.
